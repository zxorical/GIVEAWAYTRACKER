...
// ---------------------------------------------------------------------------
// Slash Commands (all return void, no direct returns from reply/editReply)
// ---------------------------------------------------------------------------
...
commands.set('active', async (interaction) => {
  ...
  if (active.length === 0) {
    await interaction.editReply({ embeds: [new EmbedBuilder().setColor(0xFFA500).setTitle('🔍 No Active Giveaways').setDescription('There are currently no active giveaways being tracked.\nCheck back later or use `/recent` to see past giveaways.')] });
    return;
  }
...
commands.set('recent', async (interaction) => {
  ...
  if (recent.length === 0) {
    await interaction.editReply({ embeds: [new EmbedBuilder().setColor(0x666666).setTitle('📭 No Giveaways Tracked').setDescription('No giveaways have been detected yet.\nThey will appear here once detected!')] });
    return;
  }
